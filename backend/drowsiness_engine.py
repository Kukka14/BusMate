...
Three-model ensemble:
  ...
  RGB CNN  (25%)  — MobileNetV3Small on face crop
  IR CNN   (15%)  — grayscale eye-state classifier

Feature extraction via MediaPipe FaceLandmarker (478 landmarks).
MinMax scaler hardcoded from nthu_features.csv column statistics.

Model files expected in:  backend/Drownsiness/
  lstm.h5            – 2-layer LSTM, input (None, 30, 5), output [alert_p, drowsy_p]
  rgb_cnn.keras      – MobileNetV3Small, input (None, 224, 224, 3)  [use .keras NOT .h5]
  ir_cnn.h5          – custom CNN, sigmoid output (0=alert/eye-open, 1=drowsy/eye-closed)
...
# Identical to the verified reference system (feature_extractor.py)
_LEFT_EYE  = [362, 385, 387, 263, 373, 380]
_RIGHT_EYE = [33,  160, 158, 133, 153, 144]

# Mouth landmarks — simpler 4-point MAR (same as reference)
...
# These match the reference: nose, chin, l-eye-outer, r-eye-outer, l-mouth, r-mouth
_POSE_IDS = [1, 152, 226, 446, 57, 287]

# ─── Canonical 3-D face model for solvePnP (mm) ──────────────────────────────
# Exact values from the reference feature_extractor.py
_FACE_3D = np.array([
...
# ─── Scaler — fitted live from nthu_features.csv (same as reference) ─────────
# Hardcoded FEAT_MIN/FEAT_MAX was wrong; the reference fits a real MinMaxScaler.
_SCALER       = None   # set in DrowsinessEngine._load()
...
# ─── Decision thresholds ──────────────────────────────────────────────────────
_DROWSY_THR      = 0.60   # > 0.60 with blink-filter (same as reference pipeline)
_CONSECUTIVE_THR = 5      # consecutive "Drowsy" frames before alert fires
...
class DrowsinessEngine:
    """
    Thread-safe drowsiness detection engine.

    Typical usage (module-level singleton in app.py):
        _dw = DrowsinessEngine()
        result = _dw.process_frame(img_bgr, session_id="drv_001")
    """
    ...
    def _load(self) -> None:
        ...
        rgb_p  = _MODEL_DIR / "rgb_cnn.keras"   # .keras format works; .h5 is broken
        ...
        try:
            ...
            # Warm-up — eliminates first-call latency spike in production
            self._lstm.predict(
            ...
            # Fit MinMaxScaler from training CSV — same approach as reference system
            # This ensures features are scaled identically to training time.
            global _SCALER
            _SCALER = self._fit_scaler(csv_p)

            self._ready = True
            print("✅ Drowsiness engine ready (LSTM + RGB-CNN + IR-CNN).")
    ...
    def _fit_scaler(csv_p: Path):
        """Fit MinMaxScaler on nthu_features.csv — exact match with training."""
    ...
    def _ear(cls, lm: Any, indices: List[int]) -> float:
        """Eye Aspect Ratio — same formula as reference feature_extractor.py."""
    ...
    def _mar(cls, lm: Any) -> float:
        """Mouth Aspect Ratio — 4-point formula from reference feature_extractor.py."""
    ...
    def _extract_features(
        ...
        """Return (EAR, MAR, pitch_deg, yaw_deg) from MediaPipe landmarks.
        Uses identical math to the reference system's feature_extractor.py.
        """
        ...
        if ok:
            ...
            # Same atan2 decomposition as the reference feature_extractor.py
    ...
    def _scale_feats(ear: float, mar: float, pitch: float, yaw: float) -> np.ndarray:
        """Scale [EAR, MAR, pitch, yaw, IR_score=0.0] using the fitted MinMaxScaler.
        IR_score is always 0.0 (no IR sensor on webcam) — same as reference system.
        """
        raw = np.array([[ear, mar, pitch, yaw, 0.0]], dtype=np.float32)
        if _SCALER is not None:
            return _SCALER.transform(raw)[0].astype(np.float32)
        return raw[0]  # fallback: unscaled

    @staticmethod
    def _prep_rgb(bgr: np.ndarray) -> np.ndarray:
        """BGR → RGB, resize 224×224, /255. Full frame (not face crop) — same as reference."""
    ...
    def _prep_ir(bgr: np.ndarray) -> np.ndarray:
        """BGR → grayscale, resize 64×64, /255. Full frame — same as reference."""
    ...
    def _infer_cnn(self, bgr: np.ndarray) -> Tuple[Optional[float], float]:
        """Run RGB CNN and IR CNN on the full frame.
        Returns (rgb_drowsy_p, ir_drowsy_p).
        IR CNN output is [alert_p, drowsy_p] (2-class softmax, same as reference).
        """
        with self._lock:
            # IR CNN: 2-class softmax [alert_p, drowsy_p] — use argmax same as reference
    ...
    def _ensemble(probs: Dict[str, Optional[float]]) -> float:
        """Weighted ensemble. Renormalises RGB+IR while LSTM is warming up."""
        lstm_p = probs.get("lstm")
        rgb_p  = probs.get("rgb")
        ir_p   = probs.get("ir", 0.0)

        if lstm_p is not None and rgb_p is not None:
            return _W_LSTM * lstm_p + _W_RGB * rgb_p + _W_IR * ir_p
        if lstm_p is not None:
            # No RGB — renormalise
            w_total = _W_LSTM + _W_IR
            return (_W_LSTM * lstm_p + _W_IR * ir_p) / w_total

        # LSTM buffer not full yet — redistribute its weight proportionally
        if rgb_p is not None:
            total = _W_RGB + _W_IR
            return (_W_RGB * rgb_p + _W_IR * ir_p) / total
        return ir_p   # last resort

    # (kept for API compat — not used internally any more)
    def _infer(self, feat_buf: deque, face_bgr: np.ndarray) -> Dict[str, Optional[float]]:
        rgb_p, ir_p = self._infer_cnn(face_bgr)
        lstm_p: Optional[float] = None
        if len(feat_buf) == _LSTM_SEQ_LEN:
            with self._lock:
                seq    = np.array(feat_buf, dtype=np.float32)[np.newaxis]
                lstm_p = float(self._lstm.predict(seq, verbose=0)[0][1])
        return {"lstm": lstm_p, "rgb": rgb_p, "ir": ir_p}

    @staticmethod
    def _old_ensemble_compat(probs):
        """Dummy — kept for import compat."""
        total = _W_RGB + _W_IR
        return (_W_RGB * probs["rgb"] + _W_IR * probs["ir"]) / total
    ...
    def process_frame(
        ...
        Returns:
            ...
            alert               bool   (consecutive_frames >= threshold)
            consecutive_frames  int
            models              {lstm, rgb, ir}  — per-model drowsy probs
        ...
        if not detection.face_landmarks:
            # No face in frame — reset consecutive counter
            state["consec"] = 0
            return {
                "ok":                 True,
                "face_detected":      False,
                ...
                "models":   {"lstm": None, "rgb": None, "ir": None},
                "features": {"ear": None, "mar": None, "pitch": None, "yaw": None},
                "bbox":     None,
            }
        ...
        # CNN inference on full frame (same as reference system — NOT face crop)
        rgb_p, ir_p = self._infer_cnn(img_bgr)

        # Build LSTM feature vector with scaler fitted from nthu_features.csv
        # IR_score is always 0.0 (no IR sensor) — same as reference system
        ...
        probs = {"lstm": lstm_p, "rgb": rgb_p, "ir": ir_p}
        score = self._ensemble(probs)

        # ── Consecutive-frame drowsiness filter ─────────────────────────────────
        if score >= _DROWSY_THR:
            state["consec"] += 1
        else:
            state["consec"] = 0
        ...
        return {
            ...
            "models": {
                "lstm": {"drowsy_prob": round(probs["lstm"], 4), "available": True}
                        if probs["lstm"] is not None
                        else {"drowsy_prob": 0.0, "available": False},
                "rgb":  {"drowsy_prob": round(probs["rgb"], 4), "available": True},
                "ir":   {"drowsy_prob": round(probs["ir"],  4), "available": True},
            },
            "features": {
                "ear":   round(ear,   4),
                "mar":   round(mar,   4),
