...
// ── Stat card ─────────────────────────────────────────────────────────────────
function StatCard({ label, value, trend, trendUp, Icon, iconBg, iconColor }) {
  return (
...
function BarChartSvg({ data }) {
  if (!data || data.length === 0) {
    return <div className="ad-chart-load"><span style={{ color: "#334155", fontSize: "0.8rem" }}>No data</span></div>;
  }
  const W = 100, H = 160, COLS = data.length;
  const maxVal = Math.max(...data.map(d => d.value), 1);
  const barW = (W / COLS) * 0.55;
  const gap  = W / COLS;

  return (
    <svg viewBox={`0 0 100 ${H}`} width="100%" height={H} preserveAspectRatio="none" style={{ display: "block" }}>
      {data.map((d, i) => {
        const bh   = (d.value / maxVal) * (H - 24);
        const x    = i * gap + (gap - barW) / 2;
        const y    = H - 20 - bh;
        const fill = d.peak ? "#f59e0b" : "#2563eb";
        return (
          <g key={i}>
            <rect x={x} y={y} width={barW} height={bh} fill={fill} rx="1" opacity="0.85"/>
            <text x={x + barW / 2} y={H - 6} textAnchor="middle"
              fill="#475569" fontSize="3.5" fontFamily="Inter,sans-serif">
              {d.day}
            </text>
          </g>
        );
      ...
      {/* baseline */}
      <line x1="0" y1={H - 20} x2="100" y2={H - 20} stroke="#162035" strokeWidth="0.5"/>
    </svg>
  );
}
...
function SignRow({ sign }) {
  const pct   = sign.total > 0 ? Math.round((sign.detected / sign.total) * 100) : 0;
  const Icon  = sign.icon === "stop" ? IcoStop : sign.icon === "warn" ? IcoWarn : sign.icon === "cross" ? IcoCheck : IcoSpeed;
  return (
...
export default function AdminDashboard() {
  ...
  const volColor = emotion.volatility === "HIGH"
    ? "#ef4444" : emotion.volatility === "MEDIUM"
    ? "#f59e0b" : "#22c55e";
        ...
        <div className="ad-content">
          ...
          <div className="ad-stats-row">
            <StatCard label="ACTIVE BUSES"     value={loading ? "—" : String(stats.active_buses ?? "—")}
              trend="+2.4%" trendUp={true}  Icon={IcoBus}   iconBg="#0e2742" iconColor="#38bdf8"/>
            ...
            <StatCard label="AVG SAFETY SCORE" value={loading ? "—" : `${stats.avg_safety_score ?? "—"}%`}
              trend="+1.2%" trendUp={true}  Icon={IcoCheck} iconBg="#0d2b1a" iconColor="#4ade80"/>
            <StatCard label="SIGN ACCURACY"    value={loading ? "—" : `${stats.sign_accuracy ?? "—"}%`}
              trend="+0.8%" trendUp={true}  Icon={IcoSign}  iconBg="#141e35" iconColor="#60a5fa"/>
          </div>

          {/* ── Row 2: Drowsiness chart + Emotion profile ── */}
          <div className="ad-row-2">
            ...
            <div className="ad-card ad-bar-card">
              ...
              {loading ? <ChartLoader /> : <BarChartSvg data={trends} />}
            </div>
          ...
          {/* ── Row 3: Sign accuracy + Scene metrics ── */}
          <div className="ad-row-2">
            ...
            <div className="ad-card">
              ...
              {loading ? <ChartLoader /> : (
                <div className="ad-sign-list">
                  {(signs.signs || []).map((s, i) => <SignRow key={i} sign={s}/>)}
                </div>
              )}
            </div>
