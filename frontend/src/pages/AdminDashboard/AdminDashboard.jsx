...
const IcoDriver   = () => <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>;
...
const IcoSettings = () => <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><path d="M19.07 4.93a10 10 0 0 0-14.14 0M4.93 19.07a10 10 0 0 0 14.14 0"/><path d="M12 2v2M12 20v2M2 12h2M20 12h2"/></svg>;
...
const IcoDots     = () => <svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24"><circle cx="5" cy="12" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="19" cy="12" r="2"/></svg>;
...
export default function AdminDashboard() {
  ...
  const mainNav = [
    ...
    { key: "fleet",     label: "Fleet Monitoring", Icon: IcoFleet   },
    { key: "drivers",   label: "Driver Analytics", Icon: IcoDriver  },
    { key: "safety",    label: "Safety Reports",   Icon: IcoShield  },
  ];
  const sysNav = [
      ...
      <aside className="ad-sidebar">
        <div className="ad-logo">
          ...
          <div className="ad-logo-text">
            <span className="ad-logo-name">BusMate</span>
            <span className="ad-logo-sub">Enterprise Fleet</span>
          </div>
        </div>

        <p className="ad-nav-section">MAIN MENU</p>
        <nav className="ad-nav">
          {mainNav.map(({ key, label, Icon }) => (
            <button key={key}
              className={`ad-nav-btn ${navKey === key ? "active" : ""}`}
        ...
        <p className="ad-nav-section" style={{ marginTop: "1.25rem" }}>SYSTEM</p>
        <nav className="ad-nav">
          {sysNav.map(({ key, label, Icon }) => (
            <button key={key}
              className={`ad-nav-btn ${navKey === key ? "active" : ""}`}
              onClick={() => setNavKey(key)}>
              <Icon /><span>{label}</span>
        ...
        <div className="ad-sidebar-user">
          ...
          <div className="ad-user-info">
            ...
            <span className="ad-user-role">Fleet Administrator</span>
          </div>
          <button className="ad-user-dots" title="Sign out" onClick={logout}><IcoDots /></button>
        </div>
      </aside>
